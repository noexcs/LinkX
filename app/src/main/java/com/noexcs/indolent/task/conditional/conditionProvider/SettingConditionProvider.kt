...
class SettingConditionProvider(private val context: Context) {
    ...
    data class SettingDef(val constant: String, val type: String)
    ...
    private val keyMap = mapOf(
        ...
        "animator_scale" to SettingDef(Settings.System.ANIMATOR_DURATION_SCALE, "float"),
        "transition_scale" to SettingDef(Settings.System.TRANSITION_ANIMATION_SCALE, "float"),
        "window_animation_scale" to SettingDef(Settings.System.WINDOW_ANIMATION_SCALE, "float"),
    ...
    fun getState(): Map<String, String> {
        ...
        for ((key, def) in keyMap) {
            val value = try {
                when (def.type) {
                    "int" -> Settings.System.getInt(cr, def.constant).toString()
                    "float" -> Settings.System.getFloat(cr, def.constant).toString()
                    "string" -> Settings.System.getString(cr, def.constant) ?: "(default)"
                    else -> "(unknown type)"
                }
